...
/** Story #004 demo — SleepTool(suffix='c'). 同 SleepTool / SleepToolB 行为。 */
@Component("sleepC")
public class SleepToolC implements Tool {

    @Override public String name() { return "sleep_c"; }
    @Override public String description() { return "Sleep for input.ms milliseconds; suffix=c"; }
    ...
    public ToolResult execute(ToolCall call, ToolExecutionContext ctx) {
        ...
        return ToolResult.builder().status(ToolResult.Status.SUCCESS).toolUseId(call.getId())
            .content("slept " + ms + "ms (suffix=c)").isError(false).build();
    }
}
