...
class LocalToolsWiringEnabledTest {
    ...
    @Test
    @DisplayName("all 4 Tool @Component beans are present in the context")
    void all4ToolBeans_present() {
        ...
        // Spring resolves Map<String, Tool> to all 4 of the @Component beans
        // (bean names readTool / writeTool / editTool / bashTool).
        assertThat(toolBeans).containsKeys("readTool", "writeTool", "editTool", "bashTool");
        assertThat(toolBeans).hasSize(4);
    }

    @Test
    @DisplayName("ToolRegistry SPI has exactly 4 tools registered by their .name()")
    void toolRegistry_has4RegisteredByName() {
        Collection<String> registered = toolRegistry.names();
        assertThat(registered)
            .as("LocalToolsAutoConfiguration.afterPropertiesSet() should "
                + "have registered exactly 4 tools by .name()")
