...
> **更新日期**:2026-09-26 — **Story #026 yaml-placeholder-resolution 已合**(567 pass / 0 fail / R-13 0 binary delta 第 11 次 / +LINGS-C03 +LINGS-C04):`PlaceholderResolver` 静态工具类(`resolvePlaceholders(Object, Path)` 递归 walk Map/List/String + brace-counting scanner 解析 4-form grammar `${X}` / `${X:default}` / `${X:${Y}}` / `$${literal}` escape + 32 层递归深度 + `Set<String> visited` 环检测)+ `YamlPlaceholderErrorCodes.LINGS_C03 / LINGS_C04`(Config 域 C 段 3/4 号 = YAML_PLACEHOLDER_UNRESOLVED / YAML_PLACEHOLDER_CYCLE)+ `AgentFactory.loadYamlAndValidate` 真接 `PlaceholderResolver.resolvePlaceholders(agent, ymlPath)`(`parseMinimalYaml` 与 `toAgentConfig` 之间),`${user.dir}` 等 env / sys-prop 占位符在 CLI + YamlWatcher hot-reload 路径上**真解析**(Spring Environment 路径一直就支持,本 Story 解决 hand-rolled 路径静默失败的 bug);17 单元测试 + 2 IT 测试(`${user.dir}` 跨 hot-reload 真接 Path + unresolved `${X}` 触发 LingsConfigException → YamlWatcher catch + rollback to previous config 守恒);5 文件改动 / 2 new(`PlaceholderResolver.java` ~250 行 + `YamlPlaceholderErrorCodes.java` ~30 行 + `PlaceholderResolverTest.java` ~250 行)+ 2 modify(`AgentFactory.java` +6 行 hook + `YamlHotReloadIT.java` +2 case + 2 helper)+ 0 新 Maven 依赖(R-13 第 11 次 PASS);`LingsConfigException` 复用 + ErrorCode 嵌入 message 模式对齐 `LinearTurnEngine.LINGS-C02`(让 `assertThatThrownBy().hasMessageContaining("LINGS-C0X")` 工作);**跨路径 parity check**(Story #026 修的 bug)`${user.dir}` 在 demo-product Spring Env 路径 + CLI hand-rolled 路径 + YamlWatcher hot-reload 路径 **三处行为完全一致**;**反模式** —— 不允许在 resolveScalar 路径上"递归加 depth 但 Set<String> visited 不变"会漏 cycle,本实现每进 `resolveOneExpression` 都 `nextVisited = new LinkedHashSet<>(parentVisited); nextVisited.add(name)` 才传下去,**测试覆盖** `cycleDetectedThrowsC04` 真造 A→B→A 嵌套默认链环。
>
...
## ✅ 已完成(Story #001—#009 + #009a/b/c/d + #017 + #018 + #019 + #020a + #020b + #020c + #009e + #022 + #023 + #024 + #025 + #025b + #025 follow-up)

详见 `README.md` 「Story 路线图」段 + `dsh_agent_design.md §13` changelog。共 30 个 Story 合入(对应 ~27 个 PR;follow-up commits 算 PR 内补丁,不独立计数):
...
| #025 follow-up | demo-product-sandbox-wiring | ✅ 合(2026-09-26,631 pass / 0 fail / R-13 0 binary delta 第 10 次 / 0 新 ErrorCode;`DemoProductApplication.readSandbox(Environment)` 私有静态 helper 模仿 `readRemoteAgents(env)` 模式 + `readSandboxList` 索引式 list helper + `mergeConfig()` 签名 +1 `AgentConfig.Sandbox` 参数(`@Value` 24-字段构造器位置 5)+ `agentConfig(Environment)` @Bean 真吃 `agent.sandbox:` YAML 5 字段(policy/runtime/working-directory/command-whitelist/domain-whitelist);Spring 启动日志 `agentConfig: sandbox bound from YAML — policy=default runtime=chroot workingDir=<cwd> cmdWhitelist(size=11) domainWhitelist(size=2)` 验证接线;3 文件改动 / ~110 行代码 + ~35 行 YAML + ~3 行 README / 0 新 Maven 依赖;补 demo-product 顶层 `agent.sandbox:` 配置示例缺失 + 接线不对称的根因(Story #025 主 commit 时 `mergeConfig()` 用 `defaults.getSandbox()`,YAML 段从未被消费)) |
| #026 | yaml-placeholder-resolution | ✅ 合(2026-09-26,567 pass / 0 fail / R-13 0 binary delta 第 11 次 / +LINGS-C03 / +LINGS-C04;`PlaceholderResolver` 静态工具类(brace-counting scanner 4-form grammar + 32 层递归 + visited Set 环检测)+ `YamlPlaceholderErrorCodes`(C 域 C03/C04 段号)+ `AgentFactory.loadYamlAndValidate` 真接 resolver(`parseMinimalYaml` 与 `toAgentConfig` 之间 hook)+ 17 单元测试(`PlaceholderResolverTest`)覆盖 happy / default / nested / escape / unclosed / missing / cycle / 多种 scalar / block-list + 2 IT 测试(`YamlHotReloadIT` `${user.dir}` 跨 hot-reload 真解 + unresolved `${X}` 触发 LingsConfigException → YamlWatcher catch + rollback);修跨路径 placeholder parity bug —— demo-product Spring Env 路径本来就支持 `${X}`,但 CLI + YamlWatcher hand-rolled 路径静默失败成 13 字符串 `${user.dir}`;5 文件改动 / 2 new (`PlaceholderResolver.java` ~250 行 + `YamlPlaceholderErrorCodes.java` ~30 行 + `PlaceholderResolverTest.java` ~250 行)+ 2 modify (`AgentFactory.java` +6 行 hook + `YamlHotReloadIT.java` +2 case + 2 helper);**LingsConfigException** 复用 + ErrorCode 嵌入 message 模式对齐 `LinearTurnEngine.LINGS-C02`(让 AssertJ `hasMessageContaining("LINGS-C0X")` 工作);0 新 Maven 依赖) |
...
| 13 | ~~**#025 follow-up**~~ | ~~`demo-product-sandbox-wiring`~~ | ~~§10.2 (examples 完整性补漏)~~ | ~~`DemoProductApplication.readSandbox(Environment)` 私有静态 helper(模仿 `readRemoteAgents(env)` 模式,5 字段全读 + INFO 日志分支)+ `readSandboxList` 索引式 list helper + `mergeConfig()` 签名 +1 `AgentConfig.Sandbox` 参数(`@Value` 24-字段构造器位置 5)+ `agentConfig(Environment)` @Bean 真吃 `agent.sandbox:` YAML 5 字段(policy/runtime/working-directory/command-whitelist/domain-whitelist);`application.yml` 加 5-字段 sandbox 段(policy + runtime + `${user.dir}` 占位符 working-directory + 11 个 command-whitelist + 2 个 domain-whitelist);`README.md` 8 features → 9 features 加 Sandbox 行 9~~ | ~~3 (application.yml + README.md + DemoProductApplication.java) + 0~~ | ~~0~~ | **✅ 已合**(2026-09-26,~110 行代码 + ~35 行 YAML + ~3 行 README / 631 pass / 0 fail / R-13 0 binary delta 第 10 次) |
| 14 | ~~**#026**~~ | ~~`yaml-placeholder-resolution`~~ | ~~§6.5 (1.5) parity fix(实测发现,2026-09-26)~~ | ~~(1) `PlaceholderResolver` 静态工具类(`resolvePlaceholders(Object, Path)` 递归 walk + brace-counting scanner 4-form grammar `${X}` / `${X:default}` / `${X:${Y}}` / `$${literal}` escape + 32 层递归深度 + `Set<String> visited` 环检测)(2) `YamlPlaceholderErrorCodes` 常量类(`LINGS_C03 = "LINGS-C03"` / `LINGS_C04 = "LINGS-C04"`,Config 域 C 段 3/4 号 = YAML_PLACEHOLDER_UNRESOLVED / YAML_PLACEHOLDER_CYCLE)(3) `AgentFactory.loadYamlAndValidate` hook `PlaceholderResolver.resolvePlaceholders(agent, ymlPath)`(`parseMinimalYaml` 与 `toAgentConfig` 之间)(4) `LingsConfigException` 复用 + ErrorCode 嵌入 message 模式对齐 `LinearTurnEngine.LINGS-C02`~~ | ~~3 new (`PlaceholderResolver.java` + `YamlPlaceholderErrorCodes.java` + `PlaceholderResolverTest.java`) + 2 modify (`AgentFactory.java` +6 行 hook + `YamlHotReloadIT.java` +2 case + 2 helper)~~ | ~~LINGS-C03 (placeholder unresolved) + LINGS-C04 (placeholder cycle)~~ | **✅ 已合**(2026-09-26,567 pass / 0 fail / R-13 0 binary delta 第 11 次 / 0 新 Maven 依赖) |

**统计**:14 个 Story / **14 已合**(#020a + #020b + #020c + #021a + #021b + #021c + #009e + #022 + #023 + #024 + #024 follow-up + #025 + #025b + #025 follow-up + **#026**)/ **0 待补** / ~80+ 个新文件(含 demo-product 15 文件 + demo-product-a2a-server ~5 文件 + `tools/cleanup-ports.sh` 1 文件 + #026 5 文件)/ 实际 +598 测试 case 累计(同前;本批 4 Story 均为 examples / wiring 修补,Stage A demo-product / demo-product-a2a-server 未引入单元测试数 —— product demo 需 `spring-boot:run` 实测,Stage B 黑盒验证待补;#026 +19 cases = 17 单元 + 2 IT → 累计 617);R-13 mitigation (d) baseline 镜像第 11 次 PASS 0 binary delta(#024 follow-up `CountDownLatch` java.util.concurrent JDK-built-in / demo-product / demo-product-a2a-server / #025 follow-up `Paths.get` / `AgentConfig.Sandbox @Value` / **#026** brace-counting 自实现 + `LinkedHashSet` + 32 层递归深度 全 JDK built-in,无新 binary 引入);**关键不变项** —— `MinimalYamlParser` (`AgentFactory.parseMinimalYaml`) 字符串 unquote / block-list 提升 / 注释忽略逻辑零改动;`AgentConfig` 不可变契约不变 / `AgentFactory` SPI 不变 / `Tool` SPI 不变 / `ToolExecutor.dispatch()` 5 步流水线不变(§4.10.1 硬规则 2)/ §4.7 PermissionPolicy / AuditLogger / Cost 域 完全兼容;0 新 Maven 依赖 / **2 新 ErrorCode**(LINGS-C03 / LINGS-C04,Config 域 C 段 3/4 号,自 #023 后首次新增 ErrorCode)。
...
并行支链(与主链无依赖):
  ...
  支链 A1(2026-09-26 增): ✅ #024 → ✅ #024 follow-up → ✅ #025 → ✅ #025b → ✅ #025 follow-up
    ...
    └ ✅ #025 follow-up demo-product-sandbox-wiring ── 已合(3 文件 / ~110 行代码 + ~35 行 YAML + ~3 行 README / R-13 0 binary delta 第 10 次,`agentConfig(Environment)` @Bean 真吃 `agent.sandbox:` YAML 5 字段)
  支链 B: ✅ #022                       (Spring AI `@AgentTool`,已合 🎉)
  支链 C: ✅ #023                       (Sub-agent,已合 🎉)
  支链 D: ✅ #025 + ✅ #025b + ✅ #025 follow-up  (Demo 产品,已合 🎉 — HTTP SSE chat + 跨 JVM A2A + sandbox wiring)
```
...
11. **2026-09-26**:**Story #025 follow-up demo-product-sandbox-wiring 已合** — `DemoProductApplication.readSandbox(Environment)` 私有静态 helper(模仿 `readRemoteAgents(env)` 模式,5 字段全读 + INFO 日志分支)+ `readSandboxList` 索引式 list helper + `mergeConfig()` 签名 +1 `AgentConfig.Sandbox` 参数(`@Value` 24-字段构造器位置 5)+ `agentConfig(Environment)` @Bean 真吃 `agent.sandbox:` YAML 5 字段(policy/runtime/working-directory/command-whitelist/domain-whitelist);3 文件改动 / ~110 行代码 + ~35 行 YAML + ~3 行 README;Spring 启动日志 `agentConfig: sandbox bound from YAML — policy=default runtime=chroot workingDir=<cwd> cmdWhitelist(size=11) domainWhitelist(size=2)` 验证接线成功;0 新 Maven 依赖 / 0 新 ErrorCode / R-13 mitigation (d) baseline 镜像第 10 次 PASS 0 binary delta(`Paths.get` + `AgentConfig.Sandbox @Value` 全部 JDK 内置,无新 binary 引入)
12. **每个 Story 合入后**:更新本文件「✅ 已完成」表 + dsh §13 changelog + `constitution.md` §10 R-XX 缓解率 + README.md Story 路线图
13. **每月 1 号**:review 本文件,确认 P0 → P2 升级 / 滞后顺序调整
